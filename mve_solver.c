...
int mve_solver(int m, int n, double *A, double *b, double *x0, double maxiter, double tol, double *x, double *E2)
{
	double one = 1.0;
	double none = -1.0;
	double zero = 0.0;
	...
	dgemv_(&Normal, &m, &n, &none, A, &m, x0, &oneI, &one, bmAx0, &oneI); /* bmAx0=b-Ax0 */
	...
	for(iter=0;iter<maxiter;iter++){ /* Main loop */
		...
		dgemm_(&Transpose,&Normal,&n, &m, &m, &one, nA, &m, Y, &m, &zero, tmpnm, &n);

		dgemm_(&Normal, &Normal, &n, &n, &m, &one, tmpnm, &n, nA, &m, &zero, E2, &n);


		dgetrf_(&n, &n, E2, &n, myworkI, &info);
		dgetri_(&n, E2, &n, myworkI, mywork, &lmywork, &info); /* we should check for info==0 */


		dgemm_(&Normal, &Normal, &m, &n, &n, &one, nA, &m , E2, &n, &zero, tmpnm, &m); /* tmpnm matriz m por n */

		dgemm_(&Normal, &Transpose, &m, &m, &n, &one, tmpnm, &m, nA, &m, &zero, Q, &m);
		...
		if(!iter){
			...
			t2=t*t; /* t=t^2 */
			dgemm_(&Normal, &Normal, &m, &m, &m, &zero, Y, &m, Y, &m, &t2, Q, &m); /* Q=t^2*Q , Q=Y*/

			t2=1/t2; /*invt=1/(t^2) */
			dgemm_(&Normal, &Normal, &m, &m, &m, &zero, Y, &m, Y, &m, &t2, Y, &m); /* Y=Y/t^2 */
		...
		dgemv_(&Transpose, &m, &n, &none, nA, &m, yh, &oneI, &zero, R1, &oneI); /* -yh*A' */
		...
		dgesv_(&m, &n, tmpmm, &m, myworkI, T, &m, &info); /* we should check for info */
		...
		dgemm_(&Normal,&Normal, &m, &n, &m, &one, tmpmm, &m, T, &m, &none, YA, &m); /* ATP = YA' */
		...
		dgemm_(&Transpose,&Normal, &n, &n, &m, &one, YA, &m, nA, &m, &zero, tmpnn, &n); /* ATP*A = tmpnn */

		dgemv_(&Transpose, &m, &n, &one, YA, &m, R23, &oneI, &one, R1, &oneI);

		dgesv_(&n, &oneI, tmpnn, &n, myworkI, R1, &n, &info); /* R1 = dx */ /* we should check for info */
		...
		dgesv_(&m, &oneI, G, &m, myworkI, dyDy, &m, &info); /* we should check for info */
