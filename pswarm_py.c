...
static PyMethodDef pswarm_methods[] = {
    {"pswarm", pswarm_py, METH_VARARGS, "Call PSwarm solver."},
    {NULL, NULL, 0, NULL}};
...
int getPyRealVector(char *txt, int n, PyObject *obj, double *x) {
  ...
  if (PyArray_DIM(array, 0) != n) {
    sprintf(msn, "Array '%s' is of wrong size. Expected %d and got %d.", txt, n,
            (int)PyArray_DIM(array, 0));
    PyErr_SetString(PyExc_ValueError, msn);
    goto clean_exit;
  }

  for (i = 0; i < n; i++) x[i] = ((double *)PyArray_DATA(array))[i];
...
void py_objfun(int n, int m, double *x, double *lb, double *ub, double *fx) {
  ...
  for (j = 0; j < m; j++) {
    for (i = 0; i < n; i++) {
      if (x[j * n + i] < lb[i] || x[j * n + i] > ub[i]) {
        PySys_WriteStdout(
            "Error computing objective function for unfeasible bound "
            "point\nReturning all as infinity.");
        for (k = 0; k < m; k++) fx[k] = 1e20;
        return;
      }
    }
  }
  ...
  if (py_x == NULL) {
    PySys_WriteStdout(
        "Error making objective argument for objective function\n");
    for (j = 0; j < m; j++) fx[j] = 1e20;
    return;
  }
  ...
  if (PyArray_DIM(result, 0) != m) {
    sprintf(msn,
            "Objective function returned bad function vector. Expected %d and "
            "got %d.",
  ...
  for (j = 0; j < m; j++) fx[j] = ((double *)PyArray_DATA(result))[j];
...
double py_outfcn(int n, int s, int iter, int gbest, struct swarm *pop) {
  PyObject *py_iter = NULL, *py_leader = NULL, *py_fx = NULL, *py_x = NULL,
           *result = NULL;
  npy_intp dim[1];
  double fx;
  ...
  if (py_outf) {
    ...
    if (py_iter == NULL) goto clean_exit;

    py_leader = PyArray_SimpleNewFromData(1, dim, PyArray_INT, &gbest);
    if (py_leader == NULL) goto clean_exit;

    py_fx =
        PyArray_SimpleNewFromData(1, dim, PyArray_DOUBLE, &(pop->fy[gbest]));
    if (py_fx == NULL) goto clean_exit;

    dim[0] = n;
    py_x =
        PyArray_SimpleNewFromData(1, dim, PyArray_DOUBLE, &(pop->y[gbest * n]));
    if (py_x == NULL) goto clean_exit;

    if ((result = PyEval_CallFunction(py_outf, "(OOOO)", py_iter, py_leader,
                                      py_fx, py_x)) == NULL) {
...
static PyObject *pswarm_py(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!", &PyDict_Type, &Problem, &PyDict_Type,
                        &Options))
  ...
  if (!py_objf || !PyFunction_Check(py_objf)) {
    PyErr_SetString(PyExc_ValueError,
                    "objf must be defined as a function object");
    goto clean_exit;
  } else {
    Py_INCREF(py_objf);
  }
  ...
  if (!py_n || !PyInt_Check(py_n)) {
    PyErr_SetString(PyExc_ValueError,
                    "The number of Variables must be provided as an integer");
    goto clean_exit;
  } else {
    Py_INCREF(py_n);
  }
  ...
  if (!py_A) {
    ...
    if (!PyList_Check(py_A)) {
      PyErr_SetString(PyExc_ValueError,
                      "A must be defined as a list of arrays (rows)");
      Py_DECREF(py_A);
      goto clean_exit;
    }
  ...
  if (py_outf && !PyFunction_Check(py_outf)) {
    PyErr_SetString(PyExc_ValueError,
                    "outputfcn must be defined as a function object");
    goto clean_exit;
  };

  if (py_outf) Py_INCREF(py_outf);
  ...
  if (py_ret == NULL) {
    PyErr_SetString(PyExc_ValueError,
                    "Unable to create the return value object");
    goto clean_exit;
  }
  if (PyDict_SetItemString(py_res, "ret", py_ret)) {
    PyErr_SetString(PyExc_ValueError,
                    "Unable to insert the return value object into dict");
    goto clean_exit;
  }
  Py_INCREF(py_ret);
  ...
  if (py_f == NULL) {
    PyErr_SetString(PyExc_ValueError,
                    "Unable to create the solution objective value object");
    goto clean_exit;
  }
  if (PyDict_SetItemString(py_res, "f", py_f)) {
    PyErr_SetString(
        PyExc_ValueError,
        "Unable to insert the solution objective value object into dict");
    goto clean_exit;
  }
  Py_INCREF(py_f);
  ...
  dim[0] = n;
  py_x =
      (PyArrayObject *)PyArray_SimpleNewFromData(1, dim, PyArray_DOUBLE, sol);
  if (py_x == NULL) {
  ...
  if (PyDict_SetItemString(py_res, "x", (PyObject *)py_x)) {
    PyErr_SetString(PyExc_ValueError,
                    "Unable to insert the solution value object into dict");
    goto clean_exit;
  }
