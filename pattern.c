...
void tangent_cone(int n, int lincons, double *A, double *b, double *x,
                  double *lb, double *ub);
#endif

extern int feasible_p(int n, double *x, int lincons, double *A, double *b,
                      double *lb, double *ub);
...
void pollstep(int n, int lincons, int pi, void (*objf)(), double *lb,
              ...
              struct poll_vector **last_success) {
  int i, j;
  ...
  if (lincons) {
    dgemv_(&Normal, &lincons, &n, &none, A, &lincons, &pop.y[pi * n], &oneI,
    ...
    if (!feasible) {
      printf(
          "Leader particle %d is not linear feasible\nThis should not "
          "happen!!!\n",
          pi);
    }
  }
  ...
  for (i = 0; i < n; i++)
    if (pop.y[pi * n + i] > ub[i] || pop.y[pi * n + i] < lb[i]) {
      printf(
          "Leader Particle is not bound feasible\nThis should not happen!!!\n");
      feasible = 0;
    }
  ...
  if (opt.vectorized) {
    ...
    while (tmp) {
      ...
      if (feasible_p(n, &vectorx[j * n], lincons, A, b, lb, ub)) j++; /* yes */
    ...
    while (tmp) {
      ...
      if (feasible_p(n, poll_point, lincons, A, b, lb, ub)) {
        if (minfx > vectorfx[j]) {
          minfx = vectorfx[j];
          minvector =
              tmp; /* don't break. We are in a non oportunistic version */
        }
        j++;
      }
      tmp = tmp->next;
  ...
  if (pop.fy[pi] > minfx) { /* a successful poll point */
    ...
    pop.fy[pi] = minfx;
    if (*last_success ==
        minvector) { /* last success vector twice, increase delta */
...
void clean_TC() {
  ...
  while (tmp1) {
    tmp2 = tmp1->next;
    if (tmp1->vector) free(tmp1->vector);
    free(tmp1);
    tmp1 = tmp2;
  }
...
void tangent_cone(int n, int lincons, double *A, double *b, double *x,
  ...
  if (ActiveThreshold > opt.EpsilonActive) ActiveThreshold = opt.EpsilonActive;

  ActiveThresholdLimit = pow(ActiveThreshold, 2.0);
  if (ActiveThresholdLimit > 0.1) ActiveThresholdLimit = 0.1;

  while (ActiveThreshold >= ActiveThresholdLimit) {
    dgemv_(&Normal, &lincons, &n, &none, A, &lincons, x, &oneI, &zero, tmpm,
           &oneI); /* tmpm = -Ax */

    NActive = 0;
    for (i = 0; i < lincons; i++)
      if (b[i] + tmpm[i] <=
          ActiveThreshold * pop.scale) /* i is epsilon active */
        NActive++;

    for (i = 0; i < n; i++) {
      if (x[i] - lb[i] <= ActiveThreshold * pop.scale) NActive++;
      if (ub[i] - x[i] <= ActiveThreshold * pop.scale) NActive++;
    }

    if (NActive > 0 &&
        NActive <=
            n) { /* we have at least a squared matrix (may be singular) */
      ...
      if (!info) {
        ...
        for (i = 0; i < NActive; i++) {
          for (j = i; j < NActive; j++) R[j * NActive + i] = tmpnm[j * n + i];
        }

        /* min(NActive,n)=NActive */

        dorgqr_(&n, &NActive, &NActive, tmpnm, &n, tau, mywork, &mylwork,
                &info); /* tmpnm = Q(n,m) */
        ...
        if (!info) {
          ...
          for (i = 0; i < n; i++) myworkI[i] = i + 1;

          dgesv_(&NActive, &n, R, &NActive, myworkI, tmpmn, &NActive,
                 &info); /* tmpmn = B' */
          ...
          if (!info) { /* solution computed */
            ...
            for (i = 0; i < n; i++) /* N = Identity */
              N[i * n + i] = 1.0;
            dgemm_(&Transpose, &Transpose, &n, &n, &NActive, &none, tmpmn,
                   &NActive, Active, &n, &one, N, &n);

            /* generate all directions -- [N -N B -B]*/

            for (i = 0; i < NActive; i++) { /* insert B and -B */
              for (j = 0; j < n; j++) tmpdir[j] = tmpmn[i + j * NActive];
            ...
            for (i = 0; i < n; i++) { /* insert N and -N */
              for (j = 0; j < n; j++) tmpdir[j] = N[i + j * n];
              tmpnrm = dnrm2_(&n, tmpdir, &oneI);
              if (tmpnrm >= opt.tol) {
                insert_TC(n, tmpdir);
              }
...
void insert_TC(int n, double *dir) {
  ...
  if (!tmp1 || !tmp2) {
    printf(
        "Unable to allocate memory for vector in tangent cone\nAborting!!\n");
    exit(1);
  }
  ...
  for (i = 0; i < n; i++) tmp2->vector[i] = -dir[i];
...
void print_poll_vector(int n, double *vector) {
  ...
  for (i = 1; i < n; i++) printf("%.2f ", vector[i]);
  printf(")\n");
}
